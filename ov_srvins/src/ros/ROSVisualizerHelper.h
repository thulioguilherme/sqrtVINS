...
#ifndef OV_SRVINS_ROSVISUALIZER_HELPER_H
#define OV_SRVINS_ROSVISUALIZER_HELPER_H
...
/**
 * @brief Helper class that handles some common versions into and out of ROS
 * formats
 */
...
public:
  ...
  get_ros_pointcloud(const std::vector<Vec3> &feats);
  ...
  get_stamped_transform_from_pose(const std::shared_ptr<ov_type::PoseJPL> &pose,
                                  bool flip_trans);
#endif
  ...
  get_stamped_transform_from_pose(std::shared_ptr<rclcpp::Node> node,
                                  const std::shared_ptr<ov_type::PoseJPL> &pose,
                                  bool flip_trans);

  /**
   * @brief Helper function that converts time in seconds to our rclcpp
   * timestamp
   ...
   */
  static void sim_save_total_state_to_file(std::shared_ptr<State> state,
...
#endif // OV_SRVINS_ROSVISUALIZER_HELPER_H
